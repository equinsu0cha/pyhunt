...
class WMIEXEC:
    def __init__(self, command='', username='', password='', domain='', hashes=None, aesKey=None, share=None,
                 noOutput=False, doKerberos=False, kdcHost=None, scanObject=None):
...
""" 
    ...
    def do_cd(self, s):
        ...
        if len(self.__outputBuffer.strip('\r\n')) > 0:
            ...
            self.__pwd = ntpath.normpath(ntpath.join(self.__pwd, s.decode("UTF-8")))
            self.execute_remote('cd ')
            self.__pwd = self.__outputBuffer.strip('\r\n')
            self.prompt = unicode(self.__pwd + '>').encode("UTF-8")
            self.__outputBuffer = u''
    ...
    def execute_remote(self, data):
        ...
        self.__win32Process.Create(command.decode("UTF-8"), self.__pwd, None)
        self.get_output()
...
if __name__ == '__main__':
    ...
    print "werd"
    try:
        executer = WMIEXEC('dir', "kiel", "wellington21", '', None, None, "ADMIN$")
        executer.run("10.1.1.34")
